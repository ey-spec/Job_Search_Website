...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function drawGrid() {
      ...
      for (var x = 0; x <= width; x += step) {
        for (var y = 0; y <= height; y += step) {
          ...
          var range = light ? 0.15 : 0.2;
          var alpha =
            base + Math.sin(x * 0.05 + y * 0.05 + tick * 0.02) * range;
    ...
    if (heroBtn && heroInput) {
      heroBtn.addEventListener("click", function () {
        ...
        window.location.href =
          path("USER/jobs.html") + "?q=" + encodeURIComponent(query);
      });
      ...
      if (openJobs.length === 0) {
        ...
        featuredGrid.querySelectorAll(".job-card").forEach(function (card) {
          card.querySelector("a").addEventListener("click", function (e) {
            ...
            const job = getJobs().find(function (j) {
              return j.id === jobId;
            });
  ...
  if (page === "jobs.html") {
    ...
    function runSearch() {
      ...
      if (!query) {
        ...
        resultsInfo.innerHTML =
          'No jobs found for <strong>"' + escapeHTML(query) + '"</strong>.';
      } else {
        resultsInfo.innerHTML =
          "Showing <strong>" +
          filtered.length +
    ...
    const categoryQuery = new URLSearchParams(window.location.search).get(
      "category",
    );
    if (categoryQuery) {
      ...
      renderJobs(filtered);
      resultsInfo.innerHTML =
        "Showing jobs in <strong>" + categoryQuery + "</strong> category.";
    }
  }
  ...
  if (page === "job-details.html") {
    ...
    document.getElementById("job-work-type").textContent =
      jobData.workType || "On-site";
    ...
    statusEl.className =
      jobData.status === "open" ? "badge-open" : "badge-closed";
    ...
    if (saveBtn) {
      ...
      const alreadySaved = saved.some(function (j) {
        return j.id === jobData.id;
      });
    ...
    const alreadyApplied = applied.some(function (j) {
      return j.id === jobData.id;
    });
    ...
    if (jobData.status === "closed") {
      disableApplyBtn("Position Closed");
      return;
    }
    if (alreadyApplied) {
      disableApplyBtn("Already Applied");
      return;
    }
  ...
  if (page === "edit-job.html") {
    ...
    const jobs = getJobs();
    const jobToEdit = jobs.find(function (j) {
      return j.id === editJobId;
    });
    ...
    document.getElementById("salary").value = jobToEdit.salary.replace(
      /[^0-9]/g,
      "",
    );
  ...
  if (page === "applicants.html") {
    ...
    if (metaEl)
      metaEl.textContent =
        job.company + " · " + (job.status === "open" ? "Open" : "Closed");

    const allApplications = JSON.parse(
      localStorage.getItem("jobApplications") || "[]",
    );
    const jobApplicants = allApplications.filter(function (a) {
      return a.jobId === viewJobId;
    });
  ...
  if (page === "profile.html") {
    ...
    if (displayRole)
      displayRole.textContent =
        role === "admin" ? "Company Admin" : "Job Seeker";
      ...
      deleteBtn.addEventListener("click", function () {
        showConfirmModal(
          ...
          function () {
            ...
            // Remove user from jobApplications (admin side)
            const allApplications = JSON.parse(
              localStorage.getItem("jobApplications") || "[]",
            );
            localStorage.setItem(
              "jobApplications",
              JSON.stringify(
                allApplications.filter(function (a) {
  ...
  function buildJobCard(job, detailsHref, showActions) {
    ...
    const applicantCount = allApplications.filter(function (a) {
  ...
  function clearError(inputId) {
    ...
    if (
      (existing && existing.classList.contains("error-message")) ||
      (existing && existing.classList.contains("success-message"))
    )
      existing.remove();
  }
      ...
      if (saved.length === 0) {
        ...
        grid.querySelectorAll(".unsave-btn").forEach(function (btn) {
          btn.addEventListener("click", function () {
            ...
            const jobId = parseInt(card.dataset.id);
            const updated = saved.filter(function (j) {
              return j.id !== jobId;
            });
        ...
        grid.querySelectorAll(".view-details-link").forEach(function (link) {
          link.addEventListener("click", function (e) {
            ...
            const card = this.closest(".job-card");
            const jobId = parseInt(card.dataset.id);
            const job = saved.find(function (j) {
              return j.id === jobId;
            });
            if (job) localStorage.setItem("selectedJob", JSON.stringify(job));
            window.location.href = "job-details.html";
